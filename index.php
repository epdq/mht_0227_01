<?php
    ...
    include_once 'lib/CrawlerStudent.php';
        ...
        foreach ($schoolList as $schoolIndex => $school) {
            ...
            for ($pageIndex = 1; $pageIndex <= $page; $pageIndex++) {
                ...
                foreach ($roomList as $listIndex => $room) {
                    ...
                    $GatherInfo = $mysql->getone('SELECT GatherID FROM house_gather WHERE GatherUrl = \'' . $roomUrl . '\'');
                    ...
                    if (!$GatherInfo) {
                        ...
                        if ($roomInfo != false) {
                            ...
                            // 插入已经采集过的URL
                            $mysql->insert('house_gather', ['GatherUrl' => $roomUrl, 'ApartmentID' => $roomId]);
