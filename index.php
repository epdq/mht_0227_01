<?php
	...
	$arrFacility = [];	// 设施数组
	$arr = $mysql->getAll('SELECT FacilityID, FacilityName FROM house_facilities WHERE 1;');
	$arrFacility = array_column($arr, 'FacilityName', 'FacilityID');

	$arrCity = [];	// 城市数组
	$arr = $mysql->getAll('SELECT CityID, AreaCnName FROM house_area WHERE 1;');
	$arrCity = array_column($arr, 'AreaCnName', 'CityID');
